...
P = 'levelup-465304'
DS = 'TALPHA_Dataset'
FB_TOKEN = get_access_token('talpha') or os.environ.get('AUUS1_META_ACCESS_TOKEN', '')
...
FB_AD_ACCOUNTS = [{'id': a['id'], 'name': a.get('name', a['id'])} for a in _dynamic_accounts] or [
    {'id': 'act_855567553811483', 'name': 'TALPHA_SA_1'},
    {'id': 'act_3534017756739334', 'name': 'TALPHA_SA_2'},
    {'id': 'act_848995974322757', 'name': 'TALPHA_AE_1'},
    {'id': 'act_719840753771124', 'name': 'TALPHA_KW_1'},
    {'id': 'act_1119368126847210', 'name': 'TALPHA_OM_1'},
    {'id': 'act_833593695771745', 'name': 'TALPHA_MIX_1'},
    {'id': 'act_703242242813144', 'name': 'TALPHA_MIX_2'},
    {'id': 'act_1503790877534258', 'name': 'TALPHA_SA_3'},
]
...
def fetch_orders_from_shop(api_key: str, shop_label: str, shop_id: str = "",
                            currency: str = "AED", last_sync_ts=None):
    """Fetch orders từ một Poscake shop (incremental với Smart Stop).

    Args:
        api_key: API key của shop.
        shop_label: Tên market (SA/AE/KW/OM/QA/BH).
        shop_id: Shop ID đã biết (nếu trống sẽ auto-discover).
        currency: Đơn vị tiền tệ của market.
        last_sync_ts: Timestamp cuối cùng đã sync — dùng để Smart Stop.
    ...
    while True:
        try:
            ...
            if resp.status_code == 500:
                log.warning(f"  {shop_label} POS API 500 error — bỏ qua shop này")
                break
        ...
        if should_stop_fetching(orders, last_sync_ts):
            log.info(f"  Smart Stop tại page {page} ({shop_label}): tất cả orders đã sync")
            break
...
def sync_all_orders(last_sync_ts=None):
    """Fetch orders từ tất cả shops TALPHA và upsert lên BigQuery."""
    all_orders, all_items = [], []
...
def sync_fb_ads(days_back=1):
    ...
    for acct in FB_AD_ACCOUNTS:
        ...
        try:
            ...
            data = resp.json()
            if 'error' in data:
                log.error(f"    Error: {data['error'].get('message','')[:150]}")
                continue
            rows = data.get('data', [])
