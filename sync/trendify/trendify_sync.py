...
Usage:
  python trendify_sync.py           # Full sync
  python trendify_sync.py --ads     # Ads only
  python trendify_sync.py --orders  # Orders only
  python trendify_sync.py --test    # Test connections
"""
...
API_KEY = os.environ.get("TRENDIFY_POSCAKE_API_KEY", "")
SHOP_ID = os.environ.get("TRENDIFY_POSCAKE_SHOP_ID", "")
P = 'levelup-465304'
DS = 'TRENDIFY_Dataset'
DEFAULT_CURRENCY = 'RON'  # ZEN8 = Middle East (AED/USD)

log_file = os.path.join(PROJECT_DIR, 'logs', f'trendify_sync_{datetime.now().strftime("%Y%m%d_%H%M")}.log')
os.makedirs(os.path.dirname(log_file), exist_ok=True)
logging.basicConfig(
    level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[logging.FileHandler(log_file, encoding='utf-8'), logging.StreamHandler(sys.stdout)]
)
log = logging.getLogger('trendify_sync')
...
FB_TOKEN = get_access_token(PROJECT_ID) or os.environ.get('TRENDIFY_META_ACCESS_TOKEN', '')
FB_AD_ACCOUNTS = get_active_account_ids(PROJECT_ID)  # fallback static list
FB_API_VERSION = 'v21.0'
FB_BM_ID = '828624026316264'  # TRENDIFY/SRN Business Manager — from n8n datadex
...
def sync_campaign_data():
    ...
    if not FB_TOKEN:
        log.error("TRENDIFY_META_ACCESS_TOKEN not set!")
        return 0
    all_campaigns = []
...
# ═══ DISCORD ═══
DISCORD_WEBHOOK = os.environ.get('TRENDIFY_DISCORD_WEBHOOK', '')

def send_discord_report(results, elapsed):
    if not DISCORD_WEBHOOK: return
    status_icon = '✅' if not results.get('errors') else '⚠️'
    embed = {
        'title': f'{status_icon} TRENDIFY Sync Report', 'color': 0x2ECC71 if not results.get('errors') else 0xE74C3C,
        'fields': [
