#!/usr/bin/env python3
"""
HNLE Full Sync — N8N-Free (STRAMARK pattern)
  [HNLE] 01 Order Sync    → fetch_all_orders() + upload_to_bq() (2 shops: AU + ME)
  [HNLE] 02 Ads Sync      → sync_fb_ads()
  [HNLE] 03 Stock + COGS  → sync_product_stock() + sync_product_cogs()

Usage:
  python hnle_sync.py           # Full sync
  python hnle_sync.py --ads     # Ads only
  python hnle_sync.py --orders  # Orders only
  python hnle_sync.py --test    # Test connections
"""
...
# HNLE has 2 shops with separate API keys
SHOPS = [
    {'id': os.environ.get('HNLE_POSCAKE_SHOP_ID', ''), 'api_key': os.environ.get('HNLE_POSCAKE_API_KEY', ''), 'name': 'HNLE_AU'},
    {'id': os.environ.get('HNLE_POSCAKE_SHOP_ID_ME', ''), 'api_key': os.environ.get('HNLE_POSCAKE_API_KEY_ME', ''), 'name': 'HNLE_ME'},
]
P = 'levelup-465304'
DS = 'HNLE_Dataset'
DEFAULT_CURRENCY = 'AED'

log_file = os.path.join(PROJECT_DIR, 'logs', f'hnle_sync_{datetime.now().strftime("%Y%m%d_%H%M")}.log')
os.makedirs(os.path.dirname(log_file), exist_ok=True)
logging.basicConfig(
    level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[logging.FileHandler(log_file, encoding='utf-8'), logging.StreamHandler(sys.stdout)]
)
log = logging.getLogger('hnle_sync')
...
FB_TOKEN = get_access_token(PROJECT_ID) or os.environ.get('HNLE_META_ACCESS_TOKEN', '')
FB_AD_ACCOUNTS = get_active_account_ids(PROJECT_ID)  # fallback static list
FB_API_VERSION = 'v21.0'
FB_BM_ID = '1305762000622659'  # HNLE Business Manager — from n8n datadex
...
def fetch_all_orders(last_sync_ts=None):
    ...
    for shop in SHOPS:
        ...
        shop_name = shop['name']
        log.info(f"  Fetching orders from {shop_name} (shop {shop_id})...")
        page = 1
        shop_count = 0
        while True:
            try:
                resp = requests.get(f"{API_URL}/shops/{shop_id}/orders",
                    params={"api_key": api_key, "page": page, "per_page": 10}, timeout=30)
                data = resp.json()
            except Exception as e:
                log.error(f"Fetch page {page} failed for {shop_name}: {e}"); break
            if data.get('message'):
                log.error(f"  {shop_name}: {data['message']}"); break
            orders = data.get('data', [])
            if not orders: break
            if should_stop_fetching(orders, last_sync_ts):
                log.info(f"  Smart Stop at page {page} for {shop_name}: all orders older than last sync")
                break
            for o in orders:
                ...
                order_row = {
                    ...
                    'order_currency': str(o.get('order_currency', DEFAULT_CURRENCY)),
        ...
        log.info(f"  {shop_name}: {shop_count} orders")
...
def sync_campaign_data():
    ...
    if not FB_TOKEN:
        log.error("HNLE_META_ACCESS_TOKEN not set!")
        return 0
    all_campaigns = []
...
# ═══ DISCORD ═══
DISCORD_WEBHOOK = os.environ.get('HNLE_DISCORD_WEBHOOK', '')

def send_discord_report(results, elapsed):
    if not DISCORD_WEBHOOK: return
    status_icon = '✅' if not results.get('errors') else '⚠️'
    embed = {
        'title': f'{status_icon} HNLE Sync Report', 'color': 0x2ECC71 if not results.get('errors') else 0xE74C3C,
        'fields': [
...
def main():
    ...
    if run_all or args.orders:
        log.info("\n[1/2] Fetching orders from Poscake...")
        try:
            ...
            log.error(f"  Orders sync failed: {e}"); results['errors'].append(f'Orders: {e}')
