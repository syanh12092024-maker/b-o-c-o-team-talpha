-- vw_true_roas — TALPHA (6 shops GCC: SA/AE/KW/OM/QA/BH)
-- Mục đích: True ROAS từ POS orders, dùng cho CMO ETL (near-realtime)
-- Dataset: TALPHA_Dataset
-- Revenue đơn vị: AED (quy đổi từ SAR/KWD/OMR/QAR/BHD)
-- POS price_divisor: 100 (cod=9900 → 99.00 AED)
-- Attribution: resolved_ad_id (p_utm_term → ad_id → adset fallback)
--   Pattern copy từ AUUS1/02_vw_fact_orders.sql lines 61-73
...
-- Tỷ giá quy về AED (nguồn: config/projects/talpha.yaml exchange_rates, 2026-02-01):
--   SAR → AED ≈ 0.977  (6850/7010 VND rates)
--   AED → AED = 1.0    (base)
--   KWD → AED ≈ 11.84  (83000/7010)
--   OMR → AED ≈ 9.52   (66700/7010)
--   QAR → AED ≈ 1.006  (7050/7010)
--   BHD → AED ≈ 9.70   (68000/7010)
-- ⚠️ SA shop: intermittent 500 errors — orders có thể thiếu
-- ⚠️ QA/BH: API keys chưa có — không có data

CREATE OR REPLACE VIEW `levelup-465304.TALPHA_Dataset.vw_true_roas` AS
...
orders_resolved AS (
    SELECT
        ...
        -- Revenue: quy về AED (chia 100 vì Poscake lưu minor units)
        CASE o.shop_label
            WHEN 'SA' THEN o.cod / 100 * 6850 / 7010    -- SAR → AED ≈ 0.977
            WHEN 'AE' THEN o.cod / 100                   -- AED (base)
            WHEN 'KW' THEN o.cod / 100 * 83000 / 7010   -- KWD → AED ≈ 11.84
            WHEN 'OM' THEN o.cod / 100 * 66700 / 7010   -- OMR → AED ≈ 9.52
            WHEN 'QA' THEN o.cod / 100 * 7050  / 7010   -- QAR → AED ≈ 1.006
            WHEN 'BH' THEN o.cod / 100 * 68000 / 7010   -- BHD → AED ≈ 9.70
            ELSE            o.cod / 100
        END                                                    AS revenue_aed
    FROM `levelup-465304.TALPHA_Dataset.sale_order` o
...
ad_to_campaign AS (
    ...
    FROM `levelup-465304.TALPHA_Dataset.fb_ads_data`
    GROUP BY ad_id
),
adset_to_campaign AS (
    ...
    FROM (
        ...
        UNION ALL
        SELECT CAST(adset_id AS STRING) AS adset_id, CAST(campaign_id AS STRING) AS campaign_id
        FROM `levelup-465304.TALPHA_Dataset.fb_adset_data`
    )
    GROUP BY adset_id
),
...
SELECT
    ...
    COALESCE(
        SUM(CASE WHEN o.status_category = 'GIAO_THANH_CONG'
                 THEN o.revenue_aed ELSE 0 END), 0
