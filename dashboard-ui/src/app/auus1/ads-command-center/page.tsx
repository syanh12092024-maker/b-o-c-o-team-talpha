...
import { RotateCw, Satellite, Layers, AlertTriangle, ChevronDown, Check } from "lucide-react";

// ── AUUS1 constants ──
import { API_BASE as BASE_URL } from "@/components/auus1/constants";
...
export default function AUUS1AdsCommandCenterPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (err.code === "ERR_NETWORK" || err.message?.includes("Network Error")) {
                setError(`FastAPI backend chưa chạy. Chạy: cd .. && uvicorn api.main:app --port 8000`);
            } else {
    ...
    // ═══ RECALCULATE SUMMARY from actual filtered campaigns ═══
    ...
    return (
        <>
            <div className="h-screen flex flex-col overflow-hidden bg-[#0F172A] text-slate-100 font-sans">
                ...
                {/* ═══ HEADER (STICKY) ═══ */}
                <header className="h-16 border-b border-slate-700 bg-[#1E293B] px-6 flex items-center justify-between shrink-0 z-50">
                    <div className="flex items-center gap-4">
                        <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                            <Satellite className="h-6 w-6 text-cyan-500" />
                            AUUS1 — ADS COMMAND CENTER
                        </h1>
                            ...
                            {isAccountDropdownOpen && (
                                <div className="absolute top-full left-0 mt-1 w-72 bg-slate-800 border border-slate-600 rounded-xl shadow-2xl z-50 overflow-hidden">
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between px-4 py-3 text-sm hover:bg-slate-700 transition ${selectedAccount === "all" ? "bg-cyan-600/20 text-cyan-300" : "text-slate-300"}`}
                                    >
                                        <span className="font-medium">🌐 All Accounts</span>
                                        {selectedAccount === "all" && <Check className="h-4 w-4 text-cyan-400" />}
                                    </button>
                                    ...
                                    {accounts.length > 0 ? (
                                        accounts.map(acc => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-slate-700 transition ${selectedAccount === acc.id ? "bg-cyan-600/20 text-cyan-300" : "text-slate-300"}`}
                                            >
                                                <div className="font-mono text-xs">{acc.id}</div>
                                                {selectedAccount === acc.id && <Check className="h-4 w-4 text-cyan-400" />}
                                            </button>
                                        ))
                                    ) : (
                                        Object.keys(groupedCampaigns).map(accId => (
                                            <button
                                                key={accId}
                                                onClick={() => { setSelectedAccount(accId); setIsAccountDropdownOpen(false); }}
                                                className={`w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-slate-700 transition ${selectedAccount === accId ? "bg-cyan-600/20 text-cyan-300" : "text-slate-300"}`}
                                            >
                                                <div className="font-mono text-xs">{accId}</div>
                                                {selectedAccount === accId && <Check className="h-4 w-4 text-cyan-400" />}
                                            </button>
                                        ))
                                    )}
                                </div>
                        ...
                        <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-600">
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md text-sm font-medium transition flex items-center gap-1.5 ${dateUsage === key ? 'bg-cyan-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                                >
                                    {label}
                                    {dateUsage === key && (
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            onClick={fetchData}
                            disabled={loading}
                            className="bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-lg shadow-cyan-500/20 border border-cyan-500 flex items-center gap-2 transition hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100"
                        >
                ...
                {/* ═══ MAIN CONTENT ═══ */}
                <main className="flex-1 overflow-auto bg-[#0F172A] p-4 space-y-4">
                    {/* Error */}
                    {error && (
                        <div className="flex items-center gap-3 bg-rose-950/50 border border-rose-800 rounded-xl p-4 text-rose-300">
                            ...
                            <button onClick={fetchData} className="ml-auto bg-rose-800 hover:bg-rose-700 px-3 py-1 rounded text-sm text-white">Retry</button>
                        </div>
                    )}
                        ...
                        Object.entries(groupedCampaigns).map(([accId, campaigns]) => {
                            ...
                            return (
                                <div key={accId} className="rounded-xl border border-slate-700 bg-[#1E293B] overflow-hidden shadow-2xl">
                                    {/* ═══ ACCOUNT HEADER (STICKY inside table scroll) ═══ */}
                                    <div className="px-4 py-3 bg-slate-800 border-b border-slate-700 flex items-center justify-between sticky top-0 z-20">
                                        <div className="flex items-center gap-3">
                                            <div className="w-2 h-8 bg-cyan-500 rounded-full"></div>
                                            <div>
                                                <h2 className="text-sm font-bold text-white uppercase tracking-wider">TK: {accId}</h2>
                                                <div className="text-xs text-slate-400">
                                                    {campaigns.length} campaigns •
                                                    Spend: <span className="text-white font-mono">{formatVND(accSpendVnd)}</span>
