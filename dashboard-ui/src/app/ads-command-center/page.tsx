...
export default function AdsCommandCenterPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Detect connection refused (backend not running)
                ...
                <header className="h-16 border-b border-slate-700 bg-[#1E293B] px-6 flex items-center justify-between shrink-0 z-50">
                                    ...
                                    {accounts.length > 0 ? (
                                        ...
                                        Object.keys(groupedCampaigns).map(accId => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-slate-700 transition ${selectedAccount === accId ? "bg-indigo-600/20 text-indigo-300" : "text-slate-300"}`}
                                            >
                                                <div className="font-mono text-xs">{accId}</div>
                                                {selectedAccount === accId && <Check className="h-4 w-4 text-indigo-400" />}
                                            </button>
                                        ))
                                    )}
                                </div>
                        ...
                        <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-600">
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md text-sm font-medium transition flex items-center gap-1.5 ${dateUsage === key ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                                >
                                    {label}
                                    {dateUsage === key && (
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            onClick={fetchData}
                            disabled={loading}
                            className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-lg shadow-indigo-500/20 border border-indigo-500 flex items-center gap-2 transition hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100"
                        >
