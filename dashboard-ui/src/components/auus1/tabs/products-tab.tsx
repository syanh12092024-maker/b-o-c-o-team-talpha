                ...
                const q0 = projectId === 'AUUS1' ? `WITH items_per_order AS (
                    ...
                    au_items AS (
                        SELECT pt.custom_id as product_code, pt.name as product_name,
                            ...
                            SAFE_DIVIDE(o.revenue_L1_lead, ipo.item_count) as item_revenue,
                    ...
                    SELECT a.product_code, a.product_name, a.market,
                        ...
                        COUNTIF(a.status_group IN ('shipping', 'success', 'returned')) as delivered,
                        COUNTIF(a.status_group = 'cancelled') as returned,
                        ROUND(SUM(a.item_revenue), 0) as revenue,
                        ROUND(SUM(a.item_cogs), 0) as cogs,
                        -- Gross = shipped_lead × 70% - COGS (AUUS1 COD: ~70% giao thành công)
                        ROUND(SUM(CASE WHEN a.status_group IN ('shipping','success','returned') THEN a.item_revenue ELSE 0 END) * 0.70 - SUM(a.item_cogs), 0) as gross_profit,
                        -- Margin = gross / (shipped × 70%)
                        ROUND(SAFE_DIVIDE(
                            SUM(CASE WHEN a.status_group IN ('shipping','success','returned') THEN a.item_revenue ELSE 0 END) * 0.70 - SUM(a.item_cogs),
                            NULLIF(SUM(CASE WHEN a.status_group IN ('shipping','success','returned') THEN a.item_revenue ELSE 0 END) * 0.70, 0)
                        ) * 100, 1) as margin,
                        COALESCE(pa.ads_spend, 0) as ads_spend,
                        ROUND(SAFE_DIVIDE(COUNTIF(a.status_group = 'cancelled'), NULLIF(COUNT(DISTINCT a.order_id),0))*100, 1) as return_rate
                    FROM all_items a
