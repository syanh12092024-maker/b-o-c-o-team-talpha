...
import { queryMarketingAdsKpis, queryMarketingDailyTrend, queryMarketingAdsByMarketer, queryMarketingMarketerPnl, queryMarketingDailyRevenue, queryMarketingDeliveredOrders } from "@/lib/bq-queries";
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                const queries = [
                    // Q0: Ads KPIs
                    queryMarketingAdsKpis(DATASET, from, to),

                    // Q1: Daily ads trend
                    queryMarketingDailyTrend(DATASET, from, to),

                    // Q2: Ads spend per marketer code
                    queryMarketingAdsByMarketer(DATASET, from, to),

                    // Q3: Marketer orders/revenue P&L
                    queryMarketingMarketerPnl(DATASET, from, to),

                    // Q4: Daily revenue for chart overlay
                    queryMarketingDailyRevenue(DATASET, from, to),

                    // Q4b: Delivered orders count from vw_fact_orders
                ...
                const testData = (results[6]?.data || []) as any[];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
                ...
                <KPICard title="🚚 Đã Giao" value={formatNumber(deliveredCount)} icon={TrendingUp}
                    subValue={totalOrders > 0 ? `${((deliveredCount / totalOrders) * 100).toFixed(1)}% đơn` : "—"}
                    status={deliveredCount > 0 ? "success" : undefined} />
                    ...
                    <table className="w-full text-xs">
                        <thead>
                            <tr className="border-b border-border text-muted-foreground">
                                ...
                                <th className="px-2 pb-2 text-right font-medium">Đã Giao*</th>
                        ...
                        <tbody>
                            {mkters.map((m) => {
                                ...
                                return (
                                    <tr key={m.marketer_name} className="border-b border-border/30 hover:bg-gray-50/50">
                                        ...
                                        <td className="px-2 py-2.5 text-right text-sky-400">—</td>
                            ...
                            <tr className="border-t-2 border-indigo-500/30 bg-indigo-500/5 font-bold">
                                ...
                                <td className="px-2 py-2.5 text-right text-sky-400 font-bold">{formatNumber(deliveredCount)}</td>
