...
import { DATASET, CURRENCY_TO_VND, FX_TO_VND, pageMktCTE, FULFILLMENT_COL, RETURN_FULFILLMENT_COL } from "../constants";

const formatCurrency = (v: number) => _fmtC(v, CURRENCY_TO_VND);
const formatMoney = (v: number) => _fmtM(v, CURRENCY_TO_VND);
import { queryMarketingAdsKpis, queryMarketingDailyTrend, queryMarketingAdsByMarketer, queryMarketingDailyRevenue, queryMarketingDailyAds, queryMarketingDailyOrders } from "@/lib/bq-queries";
                ...
                <div className="absolute top-full left-0 mt-1 z-50 rounded-xl shadow-2xl overflow-hidden w-[260px] bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-[#334155]">
                    ...
                    <div className="p-3 space-y-2.5">
                        <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30">
                            ...
                            <span className="text-xs font-semibold text-amber-600 dark:text-amber-400">Tùy chọn khoảng thời gian</span>
                        </div>
        ...
        <div ref={ref} className="relative">
            <button onClick={() => setOpen(!open)}
                className={cn(
                    ...
                    hasSelection ? "border-emerald-400 text-emerald-600 dark:text-emerald-400" : "border-gray-300 dark:border-[#334155] text-gray-700 dark:text-gray-200"
                )}>
...
const PIE_COLORS = ["#34d399", "#3b82f6", "#f59e0b", "#ef4444", "#a855f7", "#ec4899"];
...
export default function MarketingTab({ dateRange, onDateRangeChange }: MarketingTabProps) {
        ...
        async function fetchData() {
            ...
            try {
                ...
                const queries = [
                    ...
                    // Q3: Marketer P&L with both revenue_lead & delivered_revenue
                    `SELECT marketer_name,
                        ...
                        ROUND(SUM(COALESCE(${FULFILLMENT_COL}, 0)) + SUM(COALESCE(${RETURN_FULFILLMENT_COL}, 0)),0) as shipping,
                    ...
                    // Q6: Daily orders with HNLE column names
                    `SELECT report_date, marketer_name,
                        SUM(total_orders) as orders, SUM(success_orders) as success, SUM(returned_orders) as returned,
                        ROUND(SUM(COALESCE(revenue_lead, 0)),0) as revenue,
                        ROUND(SUM(COALESCE(cogs, 0)),0) as cogs,
                        ROUND(SUM(COALESCE(${FULFILLMENT_COL}, 0)) + SUM(COALESCE(${RETURN_FULFILLMENT_COL}, 0)),0) as shipping
                    FROM ${DATASET}.mart_performance_master
                    WHERE report_date BETWEEN '${from}' AND '${to}' AND marketer_name IS NOT NULL
                    GROUP BY 1, 2 ORDER BY 1 DESC, revenue DESC`,
                    // Q7: Đơn Ads
                    ...
                    // Q10: Marketer × Market breakdown
                    `WITH ${pageMktCTE(DATASET)}
                    SELECT pm.mkter as mkter_code,
                        ...
                        ROUND(SUM(CASE WHEN s.status_category = 'GIAO_THANH_CONG' THEN s.cod / 100.0 * (CASE s.order_currency WHEN 'SAR' THEN ${FX_TO_VND.SAR} WHEN 'AED' THEN ${FX_TO_VND.AED} WHEN 'KWD' THEN ${FX_TO_VND.KWD} WHEN 'AUD' THEN ${FX_TO_VND.AUD} ELSE ${FX_TO_VND.USD} END) ELSE 0 END),0) as revenue_success
                    FROM \`levelup-465304.${DATASET}.sale_order\` s
                ...
                // Aggregate P&L by canonical name
                ...
                allNames.forEach((name) => {
                    ...
                    merged.push({
                        marketer_name: name,
                        orders: pnl.orders, success: pnl.success, returned: pnl.returned,
                        revenue, revenue_success: pnl.revenue_success,
                        ads_spend: ads.ads_spend, shipping: pnl.shipping, cogs: pnl.cogs,
                        net_profit,
                        roas: Math.round(roas * 100) / 100,
                        sr: Math.round(sr * 10) / 10,
                        impressions: ads.impressions, clicks: ads.clicks,
                    });
                });
                ...
                dailyOrdersRaw.forEach((d: any) => {
                    ...
                    ordersMap.set(key, {
                        orders: prev.orders + (d.orders || 0), success: prev.success + (d.success || 0),
                        returned: prev.returned + (d.returned || 0), revenue: prev.revenue + (d.revenue || 0),
                        cogs: prev.cogs + (d.cogs || 0), shipping: prev.shipping + (d.shipping || 0),
                    });
                });
                ...
                allDailyKeys.forEach((key) => {
                    ...
                    dailyMerged.push({
                        report_date: date, marketer_name: mkter,
    ...
    const totalRevenue = displayMkters.reduce((s, m) => s + m.revenue, 0);
    const totalRevenueSuccess = displayMkters.reduce((s, m) => s + m.revenue_success, 0);
    ...
    const roasChart = displayMkters.filter(m => m.ads_spend > 0).map((m) => ({
        ...
        roas: m.roas || 0, spend: m.ads_spend, revenue: m.revenue,
    }));
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="sticky top-0 z-10 -mx-6 -mt-6 mb-0">
                <div className="flex flex-wrap items-center gap-3 px-6 py-3 bg-gray-50 dark:bg-[#1e293b] border-b border-gray-200 dark:border-[#334155]">
                    <div className="flex items-center gap-1.5 mr-2">
                        ...
                        <span className="font-bold text-base" style={{ color: C.rose }}>HNLE</span>
                    </div>
                    ...
                    <MultiSelect label="Thi truong" options={MARKETS_ALL} selected={selMarkets} onChange={setSelMarkets} />
                    <MultiSelect label="MKT" options={MKTERS_ALL} selected={selMkts} onChange={setSelMkts} />
                    <MultiSelect label="San pham" options={allProducts} selected={selProducts} onChange={setSelProducts} searchable />
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                {displayMkters.slice(0, 6).map((m, i) => {
                    ...
                    return (
                        <div key={m.marketer_name}
                            ...
                            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
                                <div>ROAS: <span className={cn("font-bold", roasTC >= 2.5 ? "text-emerald-400" : "text-amber-400")}>{roasTC.toFixed(2)}x</span></div>
                                ...
                                <div>DT TC: <span className="text-emerald-400">{formatMoney(m.revenue_success)}</span></div>
                                ...
                                <div>%Ads/DT TC: <span className={cn(adsPct <= 30 ? "text-emerald-400" : adsPct <= 50 ? "text-amber-400" : "text-rose-400")}>{adsPct.toFixed(1)}%</span></div>
                            </div>
                            {(() => {
                                ...
                                return (
                                    <div className="mt-2 border-t border-border pt-2 text-xs space-y-1">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className={cn("font-bold", profitPct >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                {profitPct.toFixed(1)}%
                                            </span>
                                        </div>
                                    </div>
                ...
                <div className="rounded-xl border border-border bg-card p-5">
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                        <BarChart data={displayMkters.filter(m => m.revenue_success > 0 || m.ads_spend > 0).map(m => {
                            ...
                            return { name: m.marketer_name?.split(" ").slice(-2).join(" ") || "?", net_pnl: net };
                        })}>
                        ...
                        <BarChart data={(() => {
                            ...
                            return Array.from(byMarket.entries())
                                ...
                                .map(([market, v]) => {
                                    ...
                                    return { name: market, net_pnl: v.revSuccess - costAlloc };
                                })
                                            ...
                                            return marketRows.map((r: any) => {
                                                ...
                                                return (
                                                    <tr key={r.market} className="border-b border-border/30 hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                                        ...
                                                        <td className={cn("px-2 py-2.5 text-right font-bold", pctGiao >= 60 ? "text-emerald-400" : "text-amber-400")}>{pctGiao.toFixed(1)}%</td>
                                                        <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(adsAlloc)}</td>
                                                        <td className="px-2 py-2.5 text-right text-amber-400">{formatMoney(cogsAlloc)}</td>
                                                        <td className="px-2 py-2.5 text-right text-cyan-400">{formatMoney(shipAlloc)}</td>
                                                        <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(totalCostR)}</td>
                                                        <td className={cn("px-2 py-2.5 text-right font-bold", netPnlR >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                            {netPnlR >= 0 ? "+" : ""}{formatMoney(netPnlR)}
                                                        </td>
                                                        <td className={cn("px-2 py-2.5 text-right font-bold", netPctR >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                            {netPctR.toFixed(1)}%
                                                        </td>
                                        ...
                                        displayMkters.map((m) => {
                                            ...
                                            return (
                                                <tr key={m.marketer_name} className="border-b border-border/30 hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                                    ...
                                                    <td className={cn("px-2 py-2.5 text-right font-bold", pctGiao >= 60 ? "text-emerald-400" : "text-amber-400")}>{pctGiao.toFixed(1)}%</td>
                                                    <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(m.ads_spend)}</td>
                                                    <td className="px-2 py-2.5 text-right text-amber-400">{formatMoney(m.cogs)}</td>
                                                    <td className="px-2 py-2.5 text-right text-cyan-400">{formatMoney(m.shipping)}</td>
                                                    <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(totalCostM)}</td>
                                                    <td className={cn("px-2 py-2.5 text-right font-bold", netPnl >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                        {netPnl >= 0 ? "+" : ""}{formatMoney(netPnl)}
                                                    </td>
                                                    <td className={cn("px-2 py-2.5 text-right font-bold", netProfitPctM >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <tr className="border-t-2 border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-500/5 font-bold">
                                                ...
                                                <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(tAds)}</td>
                                                <td className="px-2 py-2.5 text-right text-amber-400">{formatMoney(tCogs)}</td>
                                                <td className="px-2 py-2.5 text-right text-cyan-400">{formatMoney(tShip)}</td>
                                                <td className="px-2 py-2.5 text-right text-rose-400">{formatMoney(tCost)}</td>
                                                <td className={cn("px-2 py-2.5 text-right", tNet >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                    {tNet >= 0 ? "+" : ""}{formatMoney(tNet)}
                                                </td>
                                                <td className={cn("px-2 py-2.5 text-right", tNetPct >= 0 ? "text-emerald-400" : "text-rose-400")}>
                                                    {tNetPct.toFixed(1)}%
                                                </td>
