...
import { CURRENCY_TO_VND } from "../constants";
...
function ronToVnd(val: number) { return val * CURRENCY_TO_VND; }
...
const ACCOUNT_NAMES: Record<string, string> = {
    ...
    "act_850127434084549":  "Đức TK 01",
    "act_890546900299630":  "Tùng TK 06",
    "act_648920421569173":  "HNLE 6",
    "act_1344820070087553": "Tùng TK 07",
};
...
export default function AdsCommandTab() {
    ...
    const fetchData = async (fd?: string, td?: string) => {
        ...
        try {
            const res = await axios.get(`/api/hnle/realtime`, {
    ...
    const DELIVERY_SR = 0.75; // HNLE ~75% success rate
            ...
            <header className="h-14 border-b border-slate-700 bg-[#1E293B] px-5 flex items-center justify-between shrink-0 z-50">
                <div className="flex items-center gap-3">
                    <h1 className="text-lg font-bold tracking-tight text-foreground flex items-center gap-2">
                        <Satellite className="h-5 w-5 text-blue-500" /> HNLE ADS COMMAND
                    </h1>
                    {/* Account dropdown */}
