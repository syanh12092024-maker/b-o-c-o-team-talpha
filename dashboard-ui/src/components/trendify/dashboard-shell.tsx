...
export default function TRENDIFYDashboardShell() {
    ...
    // Set TRENDIFY dataset cookie on mount
    useEffect(() => {
        document.cookie = "activeDataset=TRENDIFY_Dataset; path=/;";
    }, []);
            ...
            <aside className="w-64 border-r border-border bg-white dark:bg-[#0d1117] flex flex-col shadow-sm dark:shadow-none">
                ...
                <div className="flex flex-col border-b border-border p-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-lg font-bold brand-gradient-text">TRENDIFY</span>
                            <span className="ml-2 rounded-full bg-teal-100 dark:bg-teal-500/15 px-2 py-0.5 text-[10px] font-semibold text-teal-600 dark:text-teal-400">New</span>
                        </div>
                    </div>
                    <span className="text-xs text-muted-foreground mt-1 ml-[52px]">EU Markets</span>
                </div>
                    ...
                    {TAB_ITEMS.map((item) => (
                        <button
                            ...
                            className={cn(
                                ...
                                activeTab === item.id
                                    ? "bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-500/10 dark:to-cyan-500/10 text-teal-700 dark:text-teal-400 shadow-sm border border-teal-200/60 dark:border-teal-500/20"
                                    : "text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] hover:text-foreground"
                            )}
                        >
                            <item.icon className={cn("h-4 w-4", activeTab === item.id ? "text-teal-500 dark:text-teal-400" : "")} />
                            {item.label}
                        </button>
                    ))}
                </nav>
