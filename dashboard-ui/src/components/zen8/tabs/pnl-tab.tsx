...
import { formatCurrency as _fmtC, formatMoney as _fmtM, COLORS, cn, formatNumber } from "@/lib/utils";
import { DATASET, CURRENCY_TO_VND } from "../constants";
...
import { queryPnlDailyFromMart, queryPnlDailyAds } from "@/lib/bq-queries";
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                // Q0: revenue/cogs/shipping from mart (consistent with CEO)
                // Q1: ads from pnl_view (has Mar 6-7 data)
                const [martRes, adsRes] = await Promise.all([
                    fetch("/api/query", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ query: queryPnlDailyFromMart(DATASET, from, to) }),
                    }).then((r) => r.json()).catch(() => ({ data: [] })),
                    fetch("/api/query", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ query: queryPnlDailyAds(DATASET, from, to) }),
                    }).then((r) => r.json()).catch(() => ({ data: [] })),
                ]);

                // Build ads lookup by date
                const adsMap = new Map<string, number>();
                (adsRes.data || []).forEach((r: any) => {
                    const d = r.report_date?.value || r.report_date || "";
                    adsMap.set(d, r.ads || 0);
                });

                // Merge mart + ads by date — ads now included in Q0 from pnl_flex (AED)
                const rows = martRes.data || [];
                ...
                const dailyData = rows.map((r: any) => {
                    ...
                    const cogs = r.cogs || 0;
                    const ads = r.ads || adsMap.get(date) || 0;  // prefer pnl_flex ads (AED), fallback to pnl_v2
                    const ship = r.shipping || 0;
                    const profit = rev - cogs - ads - ship;
