    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                const queries = [
                    // Q0: Market P&L summary — from mart_market_intelligence (country-level)
                    `SELECT
                        market, market_code,
                        ...
                        ROUND(SAFE_DIVIDE(SUM(gross_profit), NULLIF(SUM(revenue_success),0))*100,1) as margin,
                    ...
                    WHERE report_date BETWEEN '${from}' AND '${to}'
                        ...
                        AND market_code IN ('SA','AE','KW','BH','OM','QA')
                    GROUP BY 1,2 ORDER BY revenue DESC
                    LIMIT 15`,

                    // Q1: Best products per market (derive market from product_code prefix)
                    `SELECT
                        CASE
                            WHEN UPPER(product_code) LIKE 'KSA%' THEN 'Saudi Arabia'
                            WHEN UPPER(product_code) LIKE 'AE%' THEN 'UAE'
                            WHEN UPPER(product_code) LIKE 'KW%' THEN 'Kuwait'
                            ELSE 'Other'
                        END as market,
                    ...
                    WHERE report_date BETWEEN '${from}' AND '${to}'
                        AND product_code != 'UNKNOWN' AND product_code != 'UNMATCHED'
                    GROUP BY 1,2,3
