...
P = 'levelup-465304'
DS = 'AUUS1_Dataset'
FB_TOKEN = os.environ.get('AUUS1_META_ACCESS_TOKEN', '')
FB_AD_ACCOUNTS = ['act_2002604803685498', 'act_2026217937936451', 'act_900537852593889']
...
def fetch_orders_from_shop(api_key: str, shop_label: str, shop_id: str = ""):
    """Fetch ALL orders from a single Poscake shop."""
    if not shop_id:
    ...
    while True:
        try:
            resp = requests.get(
                ...
                params={"api_key": api_key, "page": page, "per_page": 100, "page_size": 100, "limit": 100},
                timeout=30
            )
            data = resp.json()
        except Exception as e:
...
def sync_all_orders():
    """Fetch orders from ALL shops and upload to BQ."""
    all_orders = []
    all_items = []
    
    import concurrent.futures

    with concurrent.futures.ThreadPoolExecutor(max_workers=len(POS_SHOPS)) as executor:
        futures = {
            executor.submit(fetch_orders_from_shop, shop["key"], shop["label"], shop.get("shop_id", "")): shop
            for shop in POS_SHOPS if shop["key"]
        }
        for future in concurrent.futures.as_completed(futures):
            shop = futures[future]
            try:
                orders, items = future.result()
                all_orders.extend(orders)
                all_items.extend(items)
            except Exception as exc:
                log.error(f"  {shop['label']} generated an exception: {exc}")

    if all_orders:
        upload_orders_to_bq(all_orders, all_items)
    else:
...
def _sandbox_safe_replace_table(table_name, data_rows, schema):
    """Sandbox-compatible full table replace: drop + load (no DML needed)."""
    fq = f'{P}.{DS}.{table_name}'
    dataset_ref = client.dataset(DS, project=P)
    ...
    try:
        ...
        log.warning(f"  Drop {table_name} failed (continuing): {e}")

    # Step 2: Create new table + load via WRITE_TRUNCATE
    ndjson = '\n'.join(json.dumps(r, ensure_ascii=False) for r in data_rows)
    fifty_days = int(time.time() * 1000) + (50 * 24 * 60 * 60 * 1000)
    jc = bigquery.LoadJobConfig(
        source_format=bigquery.SourceFormat.NEWLINE_DELIMITED_JSON,
        write_disposition=bigquery.WriteDisposition.WRITE_TRUNCATE,
        schema=schema,
    )
    # Create table with expiry first (sandbox requires < 60 days)
    table = bigquery.Table(client.dataset(DS, project=P).table(table_name), schema=schema)
    table.expires = datetime.utcnow() + timedelta(days=50)
    try:
        client.create_table(table)
    except Exception:
        pass  # Already exists after drop race condition

    client.load_table_from_file(
        io.BytesIO(ndjson.encode('utf-8')), fq, job_config=jc
    ).result()
    log.info(f"  {table_name}: {len(data_rows)} rows uploaded (DROP+LOAD)")


def upload_orders_to_bq(all_orders, all_items):
    """Upload orders to BQ using sandbox-safe DROP+LOAD strategy."""
    # Sale orders
    sale_order_schema = [
    ...
    _sandbox_safe_replace_table('sale_order', all_orders, sale_order_schema)
    ...
    if all_items:
        ...
        _sandbox_safe_replace_table('order_items', all_items, order_items_schema)
...
BM_ID = '377306742001877'

def get_dynamic_ad_accounts():
    accounts = list(FB_AD_ACCOUNTS)
    try:
        # Owned accounts
        resp = requests.get(f'https://graph.facebook.com/{FB_API_VERSION}/{BM_ID}/owned_ad_accounts',
            params={'fields': 'id,name', 'limit': 100, 'access_token': FB_TOKEN}, timeout=30)
        for act in resp.json().get('data', []):
            if act['id'] not in accounts: accounts.append(act['id'])
            
        # Client accounts
        resp2 = requests.get(f'https://graph.facebook.com/{FB_API_VERSION}/{BM_ID}/client_ad_accounts',
            params={'fields': 'id,name', 'limit': 100, 'access_token': FB_TOKEN}, timeout=30)
        for act in resp2.json().get('data', []):
            if act['id'] not in accounts: accounts.append(act['id'])
    except Exception as e:
        log.warning(f"  Could not auto-discover from BM: {e}")
    return accounts
...
def sync_fb_ads(days_back=1):
    ...
    active_accounts = get_dynamic_ad_accounts()
    log.info(f"  Discovered {len(active_accounts)} ad accounts from BM")
    ...
    for account_id in active_accounts:
        ...
        try:
            resp = requests.get(f'https://graph.facebook.com/{FB_API_VERSION}/{account_id}/insights',
                params={'level': 'ad', 'fields': 'ad_id,ad_name,adset_id,adset_name,campaign_id,campaign_name,spend,impressions,reach,clicks,ctr,cpc,frequency,actions,action_values,cost_per_action_type',
        ...
        try:
            resp2 = requests.get(f'https://graph.facebook.com/{FB_API_VERSION}/{account_id}/insights',
                params={'level': 'adset', 'fields': 'adset_id,adset_name,campaign_id,campaign_name,spend,impressions,reach,clicks',
                    'time_range': time_range, 'time_increment': '1', 'access_token': FB_TOKEN, 'limit': '500'}, timeout=60)
            data2 = resp2.json()
            if 'error' in data2:
...
def main():
    ...
    log.info(f"Mode: {'FULL' if run_all else 'PARTIAL'} | Test: {args.test}")
    log.info("═" * 60)
    ...
    if run_all or args.ads:
        log.info(f"\n[2/2] Syncing FB Ads (last {args.days} days)...")
        try:
